...
% Desired Position 
des_x_leader            = mission_server_data(13,:);
des_y_leader            = mission_server_data(14,:);
des_z_leader            = mission_server_data(15,:);
des_yaw_leader          = mission_server_data(16,:);
...
if Plot_command(2)
    ...
    subplot(2,2,1); hold on; 
        plot(t, des_x_leader, 'b');
        plot(t, meas_x_leader, 'r');
        hold off; grid on; grid minor;
    ylabel('m'); xlabel('Time (s)'); title('Leader X Position - Research Studio Frame/Inertial Frame');
    legend('desired','measured');

    subplot(2,2,2); hold on;
        plot(t, des_y_leader, 'b');
        plot(t, meas_y_leader, 'r');
        hold off; grid on; grid minor;
    ylabel('m'); xlabel('Time (s)'); title('Leader Y Position - Research Studio Frame/Inertial Frame');
    legend('desired','measured');

    subplot(2,2,3); hold on;
        plot(t, des_z_leader, 'b');
        plot(t, meas_z_leader, 'r');
        hold off; grid on; grid minor;
    ylabel('m'); xlabel('Time (s)'); title('Leader Z Position - Research Studio Frame/Inertial Frame');
    legend('desired','measured');

    subplot(2,2,4); hold on; 
        plot(t, des_yaw_leader, 'b');
        plot(t, meas_yaw_leader, 'r');
        hold off; grid on; grid minor;
    ylabel('rad'); xlabel('Time (s)'); title('Leader Yaw - Research Studio Frame/Inertial Frame');    
    legend('desired','measured');
end
