...
4. User input (case insensitive):
    ...
    - Land or Send Waypoints (l/w)
        ...
        The user then has to run (in a separate terminal), the 'waypoint_commands.py' file
        to send either discrete waypoints or a smooth trajectory.

        If Key 'L' is pressed on the keyboard, the virtual drone will Land.

    - Terminate at any time - Ctrl + C

5. You can monitor the status of the simulation from the QUARC Console.
   You can monitor the vrpn connection via the qlabs_vrpn_server terminal window.
