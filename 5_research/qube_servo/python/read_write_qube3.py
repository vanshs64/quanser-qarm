## example read_write_qube3.py
# This example reads and writes all available IO in the Qube Servo 3.
# This example uses a physical Qube Servo 3 device,
...
multiScope = MultiScope(
    rows=2,
    cols=3,
    title="Qube-Servo 3 Read Signals"
)
...
multiScope.axes[1].attachSignal(name='Pendulum counts/sec')
...
multiScope.axes[4].attachSignal(name='Pendulum speed (rad/s)')
...
def control_loop():
    ...
    # Set as 0 if using virtual Qube Servo 3
    # Set as 1 if using physical Qube Servo 3
    hardware = 1

    #only important if using virtual Qube. 
    # Set as 0 if using virtual Qube 3 - DC Motor
    # Set as 1 if using virtual Qube 3 - Pendulum
    pendulum = 0

    with QubeServo3(hardware = hardware, pendulum = pendulum) as myQube:
        startTime = 0
        timeStamp = 0
        def elapsed_time():
        ...
        while timeStamp < simulationTime and not KILL_THREAD:
            ...
            if count >= countMax:
                ...
                multiScope.axes[1].sample(
                    timeStamp,
                    [myQube.pendulumEncoderCounts, myQube.pendulumCountsPerSecond])
                ...
                multiScope.axes[4].sample(
                    timeStamp,
                    [myQube.pendulumPosition, myQube.pendulumSpeed])
