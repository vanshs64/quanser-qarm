@REM Batch script to stop the QUARC Server on the Traffic Light
...
for /l %%Q in (0, 1, 1) do (
    ...
    echo !TRAFFIC_LIGHT_ID!
    set "TRAFFIC_LIGHT_IP=%TRAFFIC_LIGHT_SUBNET_PREFIX%.!TRAFFIC_LIGHT_ID!"
    echo !TRAFFIC_LIGHT_IP!
    set "TRAFFIC_LIGHT_SERVER=traffic_light_target_!TRAFFIC_LIGHT_ID!.%POSTFIX_BIN%"
    echo !TRAFFIC_LIGHT_SERVER!
    start "QUARC Server" "%QUARC_DIR%quarc_run" -q -Q -t tcpip://!TRAFFIC_LIGHT_IP!:17000 !TRAFFIC_LIGHT_SERVER! -uri tcpip://!TRAFFIC_LIGHT_IP!:17001
    timeout /t 3
)