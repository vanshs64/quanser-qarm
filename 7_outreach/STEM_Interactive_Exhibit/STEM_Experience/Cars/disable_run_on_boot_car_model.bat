@REM Batch script to configure/disable the QUARC Server on the Traffic Light to run on boot
...
@REM Disabling the QCar model(s) from running on boot
@REM (Assumption: QCar model(s) had already been downloaded to the target)
...
for /l %%Q in (1, 1, 5) do (
    ...
    start "QCar Model" "%QUARC_DIR%quarc_run" -n -t tcpip://!QCAR_IP!:17000 %MODEL_MAIN%.%POSTFIX_BIN% -uri tcpip://!QCAR_IP!:17001
    timeout /t 2
)